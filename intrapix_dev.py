...
def fit_ipx_exact(x, y, mag, emag):
...
a, b, c, d = fit_ipx_exact(x, y, mag, emag)
print a, b, c, d
plt.plot(x, mag)
plt.plot(x, a*np.sin(2*np.pi*x) + b*np.cos(2*np.pi*x) + c*np.sin(2*np.pi*y) + d*np.cos(2*np.pi*y))
plt.show()
